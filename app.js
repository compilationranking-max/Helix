...
function renderFriendRows(container, names, buttonLabel, action, secondaryLabel = "") {
    ...
    if (!names.length) {
        container.innerHTML = '<div class="friends-empty">Nothing here yet.</div>';
        return;
    }

    container.innerHTML = names.map((name) => `
        <div class="friend-row">
            <span class="friend-row-avatar">${escapeHTML(friendInitials(name))}</span>
            <div class="friend-row-copy">
                <strong>${escapeHTML(name)}</strong>
                <small>Helix user</small>
            </div>
...
async function refreshFriendsNetwork() {
    ...
    try {
        ...
        renderFriendRows(friendsList, state.friends || [], "Remove", "remove");
        if (friendsCount) friendsCount.textContent = String((state.friends || []).length);
        if (friendsRequestCount) friendsRequestCount.textContent = String((state.incoming || []).length);

        if (friendsRequestList) {
            if (!state.incoming?.length) {
                friendsRequestList.innerHTML = '<div class="friends-empty">No pending requests.</div>';
            } else {
                friendsRequestList.innerHTML = state.incoming.map((request) => `
                    <div class="friend-row">
                        <span class="friend-row-avatar">${escapeHTML(friendInitials(request.from))}</span>
                        <div class="friend-row-copy">
...
async function searchHelixUsers(query) {
    ...
    if (!trimmed) {
        friendsSearchResults.innerHTML = '<div class="friends-empty">Search for a username to find people on Helix.</div>';
        return;
    }

    friendsSearchResults.innerHTML = '<div class="friends-empty">Searching the Helix network…</div>';
    ...
    try {
        ...
        if (!data.results?.length) {
            friendsSearchResults.innerHTML = '<div class="friends-empty">No matching Helix users found.</div>';
            return;
        }
        ...
        friendsSearchResults.innerHTML = '<div class="friends-empty">Search is unavailable right now.</div>';
    }
}
...
friendsSearchInput?.addEventListener("input", () => {
    searchHelixUsers(friendsSearchInput.value);
});
